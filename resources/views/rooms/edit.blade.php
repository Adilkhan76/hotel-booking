...
<head>
    ...
    <title>Edit Room - Hotel Booking</title>
            ...
            <div class="max-w-3xl mx-auto">
                <div class="bg-white shadow rounded-lg">
                    <div class="px-4 py-5 sm:px-6 border-b">
                        <h2 class="text-lg font-medium text-gray-900">Edit Room</h2>
                    </div>
                        ...
                        <form action="{{ route('rooms.update', $room->id) }}" method="POST">
                            ...
                            <div class="grid grid-cols-1 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="price_per_night" id="price_per_night" step="0.01" min="0" value="{{ $room->price_per_night }}" required 
                                           class="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                                </div>

                                <div>
                                    <label for="capacity" class="block text-sm font-medium text-gray-700 mb-1">Capacity (guests)</label>
                                    <input type="number" name="capacity" id="capacity" min="1" value="{{ $room->capacity }}" required 
                                           class="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea name="description" id="description" rows="3" 
                                              class="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">{{ $room->description }}</textarea>
                                </div>
                            ...
                            <div class="mt-6 flex justify-end space-x-3">
                                ...
                                <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-md text-sm">Update Room</button>
                            </div>
                        </form>
                    </div>
                </div>
